...
export const sameHillSpec: ExplorableSpec = {
  ...
  whatIsTheResponseSpace:
    "The form is a hill plus aspect ring (F12), not a chart-library bar or pie. Two geometries carry the space (A3, F4, F5): (1) an equal-thickness sun-rank ring — ordinal class by aspect, tick at downhill aspect (separate from pin bearing); a lone “highest” label is a fail (F9); (2) a hill profile where cold air fills the concave/floor up to the RLC = 0.4 contour. Plan frost fill runs through the apron; pocket is labeled on that outer fill, with <1% as the slope-band label on the same fill. Ring thickness/pin radius do not encode class (F1). Ghost pin is a supplement, not the only comparison (F5). Sources on the canvas (F11): Jones et al. 2004 Umpqua GIS; 10° south vs a flat site up to 25% more insolation (Jones & Duff 2007, not south vs north); Penner-Ash 2014 RLC < 0.4 = 81% of frost hours; OSU EM 8973 (rev. 2022) frost pockets via air drainage. 81% is a sourced caption, not 81.00. No legend component (F8).",
