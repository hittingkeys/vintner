...
export const occupiedPawSpec: ExplorableSpec = {
  whatMustTheLearnerSee:
    "Part A (soil water / invisible subsurface; Part C). Hidden state that must sit on the canvas (A1): occupied TAW, remaining occupied PAW, Dr vs RAW vs TAW, Cr on Willakenzie, and the shared typical-year demand. That confronts the misconception that dry-farming is a style/virtue you choose, or that terroir is a list of series names. A vine does not dry-farm in the abstract — fate is plant-available water in the occupied root zone. One Part B comparison prompt follows a short passage (not a definition card).",

  whatDoTheyManipulate:
    "The soil profile is the control (A4): drag the occupied-rooting-depth boundary on each pit. Cr is a physical cap on the Willakenzie column, not a dropdown in another panel. Young vs established is a shallower/deeper position of that same boundary (year-1 extractable depth in cm is TODO:UNVERIFIED — caption the unverified young preset; do not use a fake FAO Zr 1–2 m midpoint). Shared typical-year demand lives on the response-curve axis (same weather on both blocks) because it is not a soil property — justified separation (A4). Optional deficit drip is a per-block mode on the pit (irrigate when Dr > RAW, replace only enough to keep Dr ≤ RAW). No Run/Calculate button (A2).",

  whatIsTheResponseSpace:
    "Remaining occupied PAW versus the same seasonal demand (A3): two traces (Jory, Willakenzie) so linear drawdown vs the stall threshold (remaining → 0) is visible without further scrubbing. Current demand is a marker on that axis. Jory occupied TAW is a sourced range 25–41 cm (default map unit 25 cm; other MUs 27, 31, 41 cm) drawn as an uncertainty band — do not collapse to a fake typical. Deficit-drip ghosts floor remaining at (1 − p) × occupied TAW. The two pits are the concrete blocks; the curves are the general relationship in the same gesture (A5).",
...
export const occupiedPawLessonIntent = {
  rubric:
    "Part A (soil water holding / invisible subsurface; Part C). Light Part B: exactly one retrieval prompt in the flow after a short passage, kind comparison (not definition).",
