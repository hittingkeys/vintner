...
Lessons live in `content/` as MDX with `LessonFrontmatter`. The landing lesson is `willamette-soils.mdx` (`#/` / `willamette-soils`). `occupied-root-zone.mdx` remains at `#/occupied-root-zone`. `same-hill.mdx` is at `#/same-hill`. `$200k from Portland` is `two-hundred-k.mdx` at `#/two-hundred-k`. `content/_scaffold.mdx` remains a non-lesson compile stub.
